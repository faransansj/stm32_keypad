...
int main(void)
{
  ...
  MX_USB_DEVICE_Init();
  /* USER CODE BEGIN 2 */
	void release_key() /*release all key */
	{
		HIDkeyBoard.MODIFIER = 0x00; //shift release
		HIDkeyBoard.KEYCODE1 = 0x00; //release
	...
	void key_press(uint8_t mod,uint8_t res,uint8_t key)
	{
		//press key
		HIDkeyBoard.MODIFIER = mod;
		HIDkeyBoard.RESERVED = res;
  ...
  while (1)
  {
	uint8_t word[12] = {0x0b,0x08,0x0f,0x0f,0x12,0x2c,0x1a,0x12,0x15,0x0f,0x07,0x28};
	uint8_t mod = 0x00;
	uint8_t res = 0x00;
	uint8_t key = 0x00;
